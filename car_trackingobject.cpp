...
static void onMouse(int event, int x, int y, int, void*)
{
	...
	switch (event)
	{
	case EVENT_LBUTTONDOWN://左鍵下
		origin = Point(x, y);
		selection = Rect(x, y, 0, 0);
		selectObject = true;
		break;
	case EVENT_LBUTTONUP://左鍵上
		selectObject = false;
...
{	
	...
	while (1)
	{
		...
		if (!paused)
		{
			cvtColor(image, hsv, COLOR_BGR2HSV);//bgr to hsv

			if (trackObject)//tracking enalbe
			{
				int _vmin = vmin, _vmax = vmax;//track bar
				...
				ellipse(image, trackBox, Scalar(0, 255, 0), 2, LINE_AA);//LINE_AA
				...
				if ((trackBox.center.x < 320 && prevv == 0) || prevv == 2) {// && prevv==0)||prevv==2
		...
		char c = (char)waitKey(10);//等待10ms 若有按鍵按下 return ASCII 否則-1,若參數0 則無限等下去直到按鍵按下
		if (c == 27) //ESC 按鍵
			break;
		switch (c)
		{
		...
		case 'c':
			trackObject = 0;
			histimg = Scalar::all(0);//將scalar全部元素初始化為0
			break;
		case 'h':
			showHist = !showHist;
			if (!showHist)
